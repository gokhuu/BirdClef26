...
class AttentionPooling(nn.Module):
    ...
    def forward(self, features: torch.Tensor):
        # features: (B, C, T)
        att_logits = self.attention(features)  # (B, 1, T)
        att_weights = F.softmax(att_logits, dim=-1)  # (B, 1, T)
        pooled = (features * att_weights).sum(dim=-1)  # (B, C)
...
class BirdCLEFSED(nn.Module):
    ...
    def _encode_to_features(self, x: torch.Tensor) -> torch.Tensor:
        # (B, 1|3, n_mels, T_in) -> (B, C, T)
        x = self._normalize_and_expand(x)
        features = self.encoder(x)  # (B, C, H', W')
        features = features.mean(dim=2)  # mean over frequency -> (B, C, T)
        return features

    def _head_linear(self) -> nn.Linear:
        # Defensive: head is currently Sequential(Dropout, Linear); locate the
        # Linear by type so this still works if the head structure is tweaked.
